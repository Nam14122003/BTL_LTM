...
public class Caro extends GameLogic {
    ...
    public void addHistory(int row, int col, String playerUser) {
        History newHis = new History(row, col, playerUser);
        history.add(newHis);
        preMove = newHis;
    }
    ...
    public boolean move(int row, int col, String playerUser) {
        // nếu người này đã đánh trước đó thì không cho đánh nữa
        if (preMove != null && preMove.getPlayerUser().equals(playerUser)) {
            return false;
        }
        ...
        board[row][col] = playerUser;
        addHistory(row, col, playerUser);
        return true;
    }
    ...
    public String getLastMoveUser() {
        return history.get(history.size() - 1).getPlayerUser();
    }
